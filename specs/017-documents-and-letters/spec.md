...
- **FR-016**: System MUST apply an authorised signature to letter kinds configured to carry one.
...
- "Digital signature" in Note 19 means an authorised signature image applied to the rendered
  ...
  **If the client means a legally recognised digital signature certificate, this specification is
  wrong and must be revised**; the two differ in cost, workflow and legal effect. Flagged rather than
  assumed silently.
...
- Aadhaar is stored because the client asked for it. It is regulated personal data: it is assumed to
  be access-restricted, audit-logged on retrieval, and never rendered into a letter. If the client
  does not require Aadhaar specifically, dropping it removes a category of risk for no functional
  loss.
...
- **[NEEDS CLARIFICATION: is "Digital Signature" an image or a legal DSC?]** See Assumptions. This
  changes the feature materially.
- **[NEEDS CLARIFICATION: who may issue each commercial letter kind?]** A work order and a purchase
  order commit company money. Whether issuing one requires the approval chain of feature 016, and at
  what value threshold, is a client decision.
