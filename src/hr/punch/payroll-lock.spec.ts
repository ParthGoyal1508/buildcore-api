...
describe('isPayrollLocked', () => {
  ...
  it('leaves the current month open regardless of the lock day', () => {
    ...
    expect(
      isPayrollLocked(utc('2026-08-19T09:00:00Z'), LOCK_DAY, now, UTC),
    ).toBe(false);
  });
  ...
  it('leaves last month open before the lock day arrives', () => {
    ...
    expect(
      isPayrollLocked(utc('2026-07-31T09:00:00Z'), LOCK_DAY, now, UTC),
    ).toBe(false);
  });
  ...
  it('locks anything two or more months back', () => {
    ...
    expect(
      isPayrollLocked(utc('2026-06-30T09:00:00Z'), LOCK_DAY, now, UTC),
    ).toBe(true);
  });
  ...
  it('handles a year boundary', () => {
    ...
    expect(
      isPayrollLocked(utc('2026-12-31T09:00:00Z'), LOCK_DAY, now, UTC),
    ).toBe(true);
    expect(
      isPayrollLocked(utc('2027-01-02T09:00:00Z'), LOCK_DAY, now, UTC),
    ).toBe(false);
  });
  ...
  it('does not lock a future-dated punch', () => {
    ...
    const now = utc('2026-08-20T10:00:00Z');
    expect(
      isPayrollLocked(utc('2026-09-01T09:00:00Z'), LOCK_DAY, now, UTC),
    ).toBe(false);
  });
});
